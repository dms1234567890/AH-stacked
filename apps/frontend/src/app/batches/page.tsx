...
export default function BatchesPage() {
  ...
  const [deleteAction, setDeleteAction] = useState<'prompt' | 'shift_students' | 'delete_students'>('prompt');
    ...
    <div className="bg-red-50 border border-red-200 rounded-lg p-4 my-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-red-600 text-sm mt-1">{(error as any)?.message || 'An unexpected error occurred'}</p>
        </div>
  ...
  const selectedBatch = batches?.find((b: any) => b.id === editBatchId);
  ...
  const handleDeleteBatch = async () => {
    ...
    try {
      await deleteMutation.mutateAsync({ id: deleteBatchId, action: deleteAction !== 'prompt' ? deleteAction : undefined, targetBatchId: shiftTargetBatch || undefined });
      setDeleteBatchId('');
      ...
      <div className="max-w-6xl mx-auto bg-white rounded-lg shadow-xl p-6 md:p-10">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          <button onClick={() => setActiveSection('edit')}
            className={`px-4 py-2 rounded font-medium ${activeSection === 'edit' ? 'bg-green-600 text-white' : 'bg-green-100 text-green-700 hover:bg-green-200'}`}>
            EDIT BATCH DETAILS
          </button>
          <button onClick={() => setActiveSection('delete')}
            className={`px-4 py-2 rounded font-medium ${activeSection === 'delete' ? 'bg-red-600 text-white' : 'bg-red-100 text-red-700 hover:bg-red-200'}`}>
            DELETE BATCH
          </button>
          <button onClick={() => setActiveSection('list')}
            className={`px-4 py-2 rounded font-medium ${activeSection === 'list' ? 'bg-purple-600 text-white' : 'bg-purple-100 text-purple-700 hover:bg-purple-200'}`}>
            ALL BATCHES
          </button>
        </div>
            ...
            {batches && batches.length > 0 && (
              <div className="grid gap-3">
                {batches.map((batch: any) => (
                  <div key={batch.id} className="border rounded-lg p-4 bg-white hover:shadow-md transition">
                      ...
                      <div className="text-right">
                        <div className="flex flex-wrap gap-1 justify-end">
                          {batch.subjects?.map((s: any) => (
                            <span key={s.id} className="inline-block bg-indigo-100 text-indigo-700 text-xs px-2 py-0.5 rounded">{s.name}</span>
                          ))}
                        </div>
                      </div>
                    </div>
          ...
          <div className="p-4 bg-blue-50 rounded-lg">
            ...
            {addStep === 1 && (
              <div>
                ...
                <input type="text" value={newBatchName} onChange={(e) => setNewBatchName(e.target.value)}
                  className="border p-2 rounded w-full mb-3" placeholder="Enter batch name" />
                <button onClick={() => setAddStep(2)} disabled={!newBatchName.trim()}
                  className="bg-blue-600 text-white px-4 py-2 rounded disabled:opacity-50">Next: Choose Subjects</button>
              </div>
            )}
            ...
            {addStep === 2 && (
              <div>
                ...
                {subjects && subjects.length > 0 ? (
                  <div className="border p-2 rounded mb-3 bg-white max-h-48 overflow-y-auto grid grid-cols-2 gap-1">
                    {subjects.map((subject: any) => (
                      <label key={subject.id} className="flex items-center gap-2 text-sm p-1 hover:bg-gray-50 cursor-pointer">
                        <input type="checkbox" checked={newBatchSubjects.includes(subject.id)}
                          onChange={() => toggleSubject(subject.id, newBatchSubjects, setNewBatchSubjects)} />
                        {subject.name}
                      </label>
                    ))}
                  </div>
                ) : (
                  <p className="text-sm text-gray-500 mb-3">Loading subjects...</p>
                )}
                <div className="flex gap-2">
            ...
            {addStep === 3 && (
              <div>
                ...
                <input type="text" value={newBatchRoom} onChange={(e) => setNewBatchRoom(e.target.value)}
                  className="border p-2 rounded w-full mb-3" placeholder="Enter room number (optional)" />
                <div className="flex gap-2">
                  <button onClick={() => setAddStep(2)} className="bg-gray-500 text-white px-4 py-2 rounded">Back</button>
                  <button onClick={handleCreateBatch}
                    disabled={createMutation.isPending}
                    className="bg-green-600 text-white px-6 py-2 rounded font-medium disabled:opacity-50">
                ...
                {createMutation.isError && <p className="text-red-600 text-sm mt-2">Failed: {(createMutation.error as any)?.message}</p>}
              </div>
            )}
          </div>
        )}
            ...
            {isLoading ? renderLoading() : (
              <>
                ...
                <select value={editBatchId} onChange={(e) => {
                  ...
                  setEditBatchId(id);
                  const b = batches?.find((batch: any) => batch.id === id);
                  if (b) {
                    setEditBatchName(b.name);
                    setEditBatchRoom(b.classRoom || '');
                    setEditBatchSubjects(b.subjects?.map((s: any) => s.id) || []);
                  }
                ...
                {editBatchId && (
                  <div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Batch Name</label>
                        <input type="text" value={editBatchName} onChange={(e) => setEditBatchName(e.target.value)}
                          className="border p-2 rounded w-full" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Room Number</label>
                        <input type="text" value={editBatchRoom} onChange={(e) => setEditBatchRoom(e.target.value)}
                          className="border p-2 rounded w-full" />
                      </div>
                    </div>
                    <div className="mb-4">
                      ...
                      {subjects && subjects.length > 0 ? (
                        <div className="border p-2 rounded bg-white max-h-48 overflow-y-auto grid grid-cols-2 gap-1">
                          {subjects.map((subject: any) => (
                            <label key={subject.id} className="flex items-center gap-2 text-sm p-1 hover:bg-gray-50 cursor-pointer">
                              <input type="checkbox" checked={editBatchSubjects.includes(subject.id)}
                                onChange={() => toggleSubject(subject.id, editBatchSubjects, setEditBatchSubjects)} />
                              {subject.name}
                            </label>
                          ))}
                        </div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => { setEditBatchId(''); setEditBatchName(''); setEditBatchRoom(''); setEditBatchSubjects([]); }}
                    ...
                    {updateMutation.isError && <p className="text-red-600 text-sm mt-2">Failed: {(updateMutation.error as any)?.message}</p>}
                  </div>
                )}
              </>
            )}
                ...
                {deleteBatchId && (
                  <div>
                    ...
                    <button onClick={handleDeleteBatch}
                      ...
                      className="bg-red-600 text-white px-6 py-2 rounded font-medium disabled:opacity-50">
                    ...
                    {deleteMutation.isError && <p className="text-red-600 text-sm mt-2">Failed: {(deleteMutation.error as any)?.message}</p>}
                  </div>
                )}
              </>
            )}
