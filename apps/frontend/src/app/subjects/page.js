    ...
    return (<div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-6xl mx-auto bg-white rounded-lg shadow-xl p-6 md:p-10">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          <button onClick={() => setActiveSection('list')} className={`px-4 py-2 rounded font-medium ${activeSection === 'list' ? 'bg-green-600 text-white' : 'bg-green-100 text-green-700 hover:bg-green-200'}`}>
            View Subjects
          </button>
        </div>
        ...
        {activeSection === 'add' && (<div className="p-4 bg-blue-50 rounded-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
              <div>
                ...
                <input type="text" value={newSubjectName} onChange={(e) => handleNameChange(e.target.value)} className="border p-2 rounded w-full" placeholder="Enter subject name"/>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Subject Code (Auto-generated)</label>
                <input type="text" value={newSubjectCode} onChange={(e) => setNewSubjectCode(e.target.value)} className="border p-2 rounded w-full bg-gray-100" placeholder="Auto-generated"/>
              </div>
            </div>
            <button onClick={handleAddSubject} disabled={saving || !newSubjectName.trim()} className="bg-green-600 text-white px-4 py-2 rounded font-medium disabled:opacity-50">
        ...
        {/* Subject List */}
        {activeSection === 'list' && (<div className="p-4 bg-gray-50 rounded-lg min-h-[200px]">
            {isLoading && (<div className="flex items-center justify-center py-12">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-green-600"></div>
                <span className="ml-3 text-gray-600">Loading subjects...</span>
              </div>)}
            {isError && (<div className="bg-red-50 border border-red-200 rounded-lg p-4">
                <p className="text-red-600 text-sm">Failed to load subjects. {error?.message}</p>
                <button onClick={() => refetch()} className="mt-2 text-red-700 underline text-sm">Retry</button>
              </div>)}
            {subjects && subjects.length === 0 && (<p className="text-gray-500 text-center py-8">No subjects found. Add a new subject to get started.</p>)}
            {subjects && subjects.length > 0 && (<div className="grid gap-2">
                {subjects.map((subject) => (<div key={subject.id} className="border rounded-lg p-3 bg-white flex items-center justify-between hover:shadow-sm transition">
                    <div>
